...
import { 
  LayoutDashboard, 
  Calendar,      // لأيقونة الجدول
  Users,         // للمواعيد/المرضى
  Zap,           // للجلسات الفورية
  CreditCard,    // للمالية
  User,          // للملف الشخصي
  LogOut 
...
export default function SpecialistSidebar() {
  ...
  // قائمة روابط المختص (Specialist Pages)
  const menuItems = [
    { name: 'Dashboard', href: '/specialist', icon: LayoutDashboard },
    { name: 'My Schedule', href: '/specialist/schedule', icon: Calendar },
    { name: 'Appointments', href: '/specialist/appointments', icon: Users },
    { name: 'Instant Sessions', href: '/specialist/instant', icon: Zap },
    { name: 'Financials', href: '/specialist/financials', icon: CreditCard },
    { name: 'My Profile', href: '/specialist/profile', icon: User },
  ];

  return (
    // الخلفية كحلية (#352C5E) ثابتة - نفس تصميم الأدمن بالضبط
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link 
              key={item.href} 
              href={item.href}
              // تصميم الرابط: نفس تأثيرات الأدمن تماماً
              className={`
